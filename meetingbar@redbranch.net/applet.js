...
class MeetingBarApplet extends Applet.Applet {
    ...
    on_refresh_button_clicked() {
        ...
        Main.notify('Meeting Bar', 'Refreshing calendars...');
    ...
    _update_calendar() {
        ...
        if (!this.ical_urls || this.ical_urls.trim() === '') {
            this._label.set_text('No calendars configured');
            return;
        }
                ...
                try {
                    ...
                    if (success) {
                        ...
                        if (this.manualRefresh) {
                            this.refreshStatus.push({ url: url, status: 'success', message: 'Loaded successfully' });
                        }
                    } else {
                        global.log('Failed to fetch calendar from: ' + url);
                        if (this.manualRefresh) {
                            this.refreshStatus.push({ url: url, status: 'error', message: 'Failed to fetch' });
                        }
                    ...
                    if (this.manualRefresh) {
                        this.refreshStatus.push({ url: url, status: 'error', message: 'Error: ' + e.message });
                    }
                    // Still decrement the counter
                    this._on_fetch_complete();
                }
    ...
    _show_refresh_results() {
        ...
        let message = `Refreshed ${totalCalendars} calendar${totalCalendars !== 1 ? 's' : ''}:\n`;
        message += `✓ ${successCount} successful, ✗ ${errorCount} failed\n`;
        message += `Found ${totalEvents} event${totalEvents !== 1 ? 's' : ''} today\n\n`;
        ...
        // Show notification
        Main.notify('Meeting Bar Refresh Complete', message);
    }
    ...
    _parse_ical(icalData, url) {
        try {
            ...
            if (this.manualRefresh) {
                ...
                if (statusEntry) {
                    statusEntry.message = `Loaded successfully (${events.length} event${events.length !== 1 ? 's' : ''})`;
            ...
            if (this.manualRefresh) {
                this.refreshStatus.push({ url: url, status: 'error', message: 'Parse error: ' + e.message });
            }
    ...
    _find_next_meeting(events) {
        ...
        if (upcomingEvents.length === 0) {
            this._label.set_text('No upcoming meetings today');
            this.nextMeeting = null;
            return;
        }
    ...
    _update_tooltip(events) {
        if (events.length === 0) {
            this.set_applet_tooltip('No events today');
            return;
        }
        ...
        if (todayEvents.length > 0) {
            agendaLines.push('Today\'s Agenda:', '');
        ...
        if (tomorrowEvents.length > 0) {
            ...
            agendaLines.push('Tomorrow\'s Agenda:', '');
    ...
    _populate_menu() {
        ...
        if (this.allEvents.length === 0) {
            let item = new PopupMenu.PopupMenuItem('No events today', { reactive: false });
        ...
        if (todayEvents.length > 0) {
            let header = new PopupMenu.PopupMenuItem("Today's Agenda", { reactive: false });
        ...
        if (tomorrowEvents.length > 0) {
            ...
            let header = new PopupMenu.PopupMenuItem("Tomorrow's Agenda", { reactive: false });
            header.label.set_style('font-weight: bold; color: white;');
            this.menu.addMenuItem(header);
            this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
    ...
    _add_event_to_menu(event, now) {
        ...
        // Add status prefix
        if (status === 'CANCELLED') {
            plainLine = '[CANCELLED] ' + plainLine;
        } else if (status === 'TENTATIVE') {
            plainLine = '[TENTATIVE] ' + plainLine;
        }
